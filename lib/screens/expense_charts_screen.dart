...
class _CategoryPieChart extends StatelessWidget {
  ...
  List<PieChartSectionData> _generateChartData(Map<String, double> data) {
    final colors = [Colors.blue, Colors.red, Colors.green, Colors.orange, Colors.purple, Colors.cyan, Colors.brown];
    int colorIndex = 0;
...
class _MonthlyBarChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        children: [
          const Text('Expenses Over Time (Last 6 Months)', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          const SizedBox(height: 32),
          Expanded(
            child: BarChart(
              BarChartData(
  ...
  Map<String, double> _getMonthlyData(ExpenseProvider provider) {
    ...
    // We sort or ensure logical order. For simplicity in this mockup, we just return the mapped values.
    return monthlyData;
  }
}
